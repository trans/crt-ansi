...
describe CRT::Ansi::Panel do
  describe "#border" do
    it "draws a border on .draw" do
      r = renderer
      r.panel(0, 0, w: 5, h: 3).border.draw
    ...
    it "supports border styles" do
      r = renderer
      r.panel(0, 0, w: 5, h: 3).border(CRT::Ansi::Border::Rounded).draw
      r.cell(0, 0).grapheme.should eq("╭")
      r.cell(4, 0).grapheme.should eq("╮")
    end
  end
  ...
  describe "#fill" do
    it "fills interior within border" do
      ...
      r.panel(0, 0, w: 5, h: 3).border.fill(bg).draw
    ...
    it "fills full area without border" do
      ...
      r.panel(0, 0, w: 4, h: 2).fill(bg).draw
    ...
    it "fills with a Style::Char" do
      r = renderer
      r.panel(0, 0, w: 5, h: 3).border.fill(CRT::Ansi::Style::Char.new('·')).draw
  ...
  describe "#text" do
    it "writes left-aligned text inside border" do
      r = renderer
      r.panel(0, 0, w: 10, h: 3).border.text("Hi").draw
    ...
    it "writes center-aligned text" do
      r = renderer
      r.panel(0, 0, w: 12, h: 3).border.text("Hi", align: CRT::Ansi::Align::Center).draw
    ...
    it "writes right-aligned text" do
      r = renderer
      r.panel(0, 0, w: 12, h: 3).border.text("Hi", align: CRT::Ansi::Align::Right).draw
    ...
    it "truncates text that exceeds width" do
      r = renderer(15, 5)
      r.panel(0, 0, w: 8, h: 3).border.text("Hello World").draw
    ...
    it "writes text without border using full area" do
      r = renderer
      r.panel(0, 0, w: 10, h: 3).text("Hello").draw
  ...
  describe "Wrap::None with newlines" do
    it "splits on explicit newlines" do
      r = renderer(20, 10)
      r.panel(0, 0, w: 12, h: 5).border.text("line one\nline two").draw
    ...
    it "truncates long lines without wrapping" do
      r = renderer(20, 10)
      r.panel(0, 0, w: 7, h: 4).border.text("abcdefghij\nxy").draw
  ...
  describe "Wrap::Word" do
    it "wraps text at word boundaries" do
      r = renderer(20, 10)
      r.panel(0, 0, w: 12, h: 6).border.text("one two three four five", wrap: CRT::Ansi::Wrap::Word).draw
    ...
    it "breaks long words" do
      r = renderer(20, 10)
      r.panel(0, 0, w: 7, h: 5).border.text("abcdefghij", wrap: CRT::Ansi::Wrap::Word).draw
    ...
    it "clips text that exceeds height" do
      r = renderer(20, 10)
      r.panel(0, 0, w: 8, h: 4).border.text("a b c d e f g h", wrap: CRT::Ansi::Wrap::Word).draw
  ...
  describe "Wrap::Char" do
    it "wraps at exact character width" do
      r = renderer(20, 10)
      r.panel(0, 0, w: 7, h: 5).border.text("abcdefghij", wrap: CRT::Ansi::Wrap::Char).draw
    ...
    it "does not break at word boundaries" do
      r = renderer(20, 10)
      r.panel(0, 0, w: 7, h: 5).border.text("ab cd ef", wrap: CRT::Ansi::Wrap::Char).draw
    ...
    it "preserves explicit newlines" do
      r = renderer(20, 10)
      r.panel(0, 0, w: 7, h: 5).border.text("abc\ndef", wrap: CRT::Ansi::Wrap::Char).draw
  ...
  describe "VAlign" do
    it "top-aligns by default (clips bottom)" do
      r = renderer(20, 10)
      r.panel(0, 0, w: 10, h: 4).border.text("a\nb\nc\nd\ne").draw
  ...
  describe "alignment-aware clipping" do
    it "clips right side for left-aligned overflow" do
      r = renderer(20, 5)
      r.panel(0, 0, w: 7, h: 3).border.text("abcdefgh").draw
  ...
  describe "Style::Text" do
    it "renders styled spans" do
      ...
      r.panel(0, 0, w: 10, h: 3).border.text(text).draw
    ...
    it "preserves styles through word wrap" do
      ...
      r.panel(0, 0, w: 9, h: 5).border.text(text, wrap: CRT::Ansi::Wrap::Word).draw
    ...
    it "splits styled text on newlines" do
      ...
      r.panel(0, 0, w: 10, h: 4).border.text(text).draw
  ...
  describe "#shadow" do
    it "draws shadow cells offset from box" do
      r = renderer
      r.panel(1, 1, w: 4, h: 3).border.shadow.draw
  ...
  describe "chaining order independence" do
    it "produces the same result regardless of chain order" do
      ...
      r1 = renderer
      r1.panel(0, 0, w: 10, h: 4).border.fill(bg).text("Hi").draw

      r2 = renderer
      r2.panel(0, 0, w: 10, h: 4).text("Hi").fill(bg).border.draw
  ...
  describe "#text with pad" do
    it "adds horizontal padding to text" do
      r = renderer
      r.panel(0, 0, w: 14, h: 3).border.text("Hi", pad: 2).draw
